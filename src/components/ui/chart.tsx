...
import type {
  Config,
  ExtendedTooltipPayload,
  ExtendedLegendPayload,
} from "@/types/chart"
...
>((props, ref) => {
  ...
  const { config } = useChart() as { config: Config }
  const safePayload: ExtendedTooltipPayload[] = Array.isArray(payload)
    ? (payload as ExtendedTooltipPayload[])
    : []
