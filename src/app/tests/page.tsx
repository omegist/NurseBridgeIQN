          ...
          {parts.map((part, index) => {
            ...
            if (theme === 'light') {
              return (
                ...
                >
                  <div onClick={isLocked ? handlePartClick : undefined} className="relative block cursor-pointer">
                    {isLocked && (
                      <div className="absolute inset-0 bg-black/60 rounded-2xl flex flex-col items-center justify-center z-10 p-4">
                        ...
                        <Button onClick={handlePartClick} className="w-full bg-accent hover:bg-accent/90">
                           Unlock Full App
                        </Button>
                      </div>
                    )}
                    <Link href={isLocked ? '#' : part.href} className={isLocked ? 'pointer-events-none' : ''}>
